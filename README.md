...
CC 的 context window 是有限的。聊久了 session 会变得很大（14MB+ 的 JSONL），CC 开始变慢、容易 OOM。需要定期轮换到新 session。

**什么时候需要轮换：**
- CC 内置的 context 自动压缩（compaction）在大部分情况下足够，不需要频繁手动轮换
- 只有 OOM 杀进程、CC 异常退出、或 session 积累到明显变慢时才需要

**轮换前的准备（让 CC 自己做）：**

在旧 session 还活着的时候，让 CC 把交接材料准备好：
1. 让 CC 写一份 session 摘要，保存到约定路径（如 `session_summaries/pending_*.md`）
2. 摘要应该包含：当前在聊什么、重要上下文、用户最近的状态

**启动脚本自动注入摘要：**
- 启动脚本检查有没有待处理的 session 摘要文件
- 如果有，用 `--append-system-prompt-file` 把摘要注入新 session 的系统提示
- 同时从旧 session 的 JSONL 提取最后 20 条对话，附在摘要后面
- 启动后延时 20 秒注入一条 nudge 让 CC 主动打招呼

**通过前端终端视图执行轮换（已验证的稳定方案）：**

不需要 SSH 进服务器，直接在手机/网页的终端视图里操作：
1. 在终端视图观察 CC 当前状态，确认摘要已写好
2. 用预制指令栏的 **⌃C** 按钮中断 CC 进程（需 PIN 确认，10 秒内再按免 PIN）
3. CC 退出后会提示 "Press Ctrl-C again to exit"，**再按一次** ⌃C 彻底退出
4. 用 **start CC** 预制指令重新拉起（`bash start_cc.sh`）
5. 等待 CC 启动完成（约 20 秒），nudge 会自动注入
6. 如果 nudge 没有被处理（CC 在 prompt 处没动静），手动发一个空 Enter 或任意消息触发

终端视图实时同步 tmux 画面，整个过程可视可控。

**注意事项：**
- nudge 注入的时机依赖 CC 初始化完成，偶尔会因为 CC 还没 ready 导致 Enter 被吞，手动补发即可
- 如果不需要新 session 而是想恢复上一个，用 **claude --continue** 预制指令
- 新 session 的 CC 虽然有摘要，但质感和记忆深度跟聊了几百轮的旧 session 差很多——这是 context 压缩的固有代价
