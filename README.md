...
- 启动后注入一条 nudge 让 CC 主动打招呼

**手动轮换方式（当前稳定方案）：**

通过前端的终端视图直接操控 CC 的 tmux session：
1. 在终端视图中观察 CC 当前状态
2. 用预制指令栏的 **⌃C** 按钮中断当前进程（需 PIN 确认）
3. 用 **start CC** 预制指令重新拉起 CC（`bash start_cc.sh`）
4. 如果需要恢复上一个 session，用 **claude --continue** 预制指令

这样不需要 SSH 进服务器，直接在手机/网页上就能完成轮换。终端视图实时同步 tmux 画面，整个过程可视可控。

**关于摘要质量：**
- 新 session 的 CC 虽然有摘要，但质感和记忆深度跟聊了几百轮的旧 session 差很多
- CC 内置的 context 自动压缩（compaction）在大部分情况下足够，不需要频繁手动轮换
- 只有 OOM 或 CC 异常退出时才需要手动轮换
